...
const { getJobDescription } = require('./html');
...
function calculateScore(jobHTML, resumeText) {
	// returns an integer score from 0 to 100, inclusive
	const jobText = getJobDescription(jobHTML);
	const jobTokens = toTokens(jobText);
	const resumeTokens = toTokens(resumeText);
	return simpleSimilarity(jobTokens, resumeTokens);
}
